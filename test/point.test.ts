...
describe('bls12-381 Point', () => {
  ...
  describe('Point with Fp2 coordinates', () => {
    it('Point equality', () => {
      fc.assert(
        fc.property(
          ...
          fc.array(fc.array(FC_BIGINT, 2, 2), 3, 3),
          ([x1, y1, z1], [x2, y2, z2]) => {
            const p1 = new PointG2(new Fp2(x1), new Fp2(y1), new Fp2(z1));
            const p2 = new PointG2(new Fp2(x2), new Fp2(y2), new Fp2(z2));
            expect(p1.equals(p1)).toBe(true);
            expect(p2.equals(p2)).toBe(true);
            expect(p1.equals(p2)).toBe(false);
            expect(p2.equals(p1)).toBe(false);
    ...
    it('should be placed on curve vector 1', () => {
      const a = new PointG2(new Fp2([0n, 0n]), new Fp2([1n, 0n]), new Fp2([0n, 0n]));
      a.assertValidity();
    });
    it('should be placed on curve vector 2', () => {
      const a = new PointG2(
    ...
    it('should not be placed on curve vector 1', () => {
      const a = new PointG2(new Fp2([0n, 0n]), new Fp2([1n, 0n]), new Fp2([1n, 0n]));
      expect(() => a.assertValidity()).toThrowError();
    });
    it('should not be placed on curve vector 2', () => {
      const a = new PointG2(
